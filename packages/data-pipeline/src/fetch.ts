...
async function fetchWithRetry(
  ...
  const url = `${ARCHIVE_URL}?latitude=${lat}&longitude=${lng}&start_date=${START_DATE}&end_date=${END_DATE}&daily=temperature_2m_mean,pressure_msl_mean&timezone=UTC`;
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (status === 429 || status >= 500) {
      if (attempt === MAX_RETRIES) {
        throw new Error(
          `Failed after ${MAX_RETRIES} retries for (${lat}, ${lng}): HTTP ${status}`,
        );
      }
      ...
      console.log(
        `\nHTTP ${status} for (${lat}, ${lng}), retrying in ${backoff / 1000}s (attempt ${attempt + 1}/${MAX_RETRIES})`,
      );
      await sleep(backoff);
      continue;
    }
    ...
    throw new Error(
      `Unexpected HTTP ${status} for (${lat}, ${lng}): ${await res.text()}`,
    );
  }
  ...
  for (let i = startIndex; i < total; i++) {
    ...
    const msPerLocation =
      completedSinceResume > 0
        ? (Date.now() - startTime) / completedSinceResume
        : DELAY_MS + 3000;
    ...
    const daily = await fetchWithRetry(loc.lat, loc.lng);
    data.set(i, daily);
