...
function tempForLatMonth(lat: number, month: number): number {
  ...
  const northernOffset = lat >= 0 ? Math.cos(((month - 7) / 6) * Math.PI) : Math.cos(((month - 1) / 6) * Math.PI);
...
async function main() {
  ...
  for (let year = 1940; year <= 2024; year++) {
    for (let m = 1; m <= 12; m++) {
      ...
      const days: Record<string, Array<{ t1: number | null; p1: number | null; t2: number | null; p2: number | null }>> = {};
